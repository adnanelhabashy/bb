  ...
  const host = createFakePluginHost({
    ...
    sdk: {
      files: {
        ...
        read: async (args) => {
          if (!("path" in args))
            throw new Error("Vault read requires a host path");
    ...
    sdk: {
      files: {
        async listPaths(args) {
          if (!("path" in args))
            throw new Error("Vault operation requires a host path");
...
describe("Docs vault operations", () => {
  ...
  it.each([
    ...
    { kind: "host" as const, hostId: "host_remote", path: "/shared/plan.md" },
    {
    ...
    "opens and saves $kind references through core file operations",
    async (file) => {
      const host = createFakePluginHost({
        pluginId: "simple-notes",
        sdk: {
          files: {
            mkdir: async () => ({ ok: true as const }),
            read: async () => ({
              path: "/resolved/plan.md",
              content: "# Plan",
              contentEncoding: "utf8" as const,
              sizeBytes: 6,
              sha256: "original",
            }),
            experimental_resolveResource: async () => ({
              target: file,
              path: "plan.md",
              absolutePath: "/resolved/plan.md",
              rootPath: "/resolved",
              baseUrl: "/preview",
              url: "/preview/plan.md",
              expiresAtMs: Date.now() + 60_000,
      ...
      await simpleNotes(host.bb);
      const opened = await host.harness.callRpc("openFile", { file });
      ...
      expect(
        host.harness.sdk.callsTo("files.experimental_resolveResource"),
      ).toEqual([[{ target: file }]]);
      await expect(
      ...
      ).resolves.toEqual({ outcome: "conflict", currentSha256: "concurrent" });
  ...
  it("rejects thread-storage paths that escape the confined root at the RPC boundary", async () => {
    const { harness } = await loadNotebook({ "plan.md": "# Plan" });
