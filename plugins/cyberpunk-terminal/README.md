...
- `server.ts` — the backend: a todo store in `bb.storage.kv`, RPC methods
  for the page, a `bb cyberpunk-terminal` CLI command, a setting, and a realtime signal
...
- `skills/example-todos/SKILL.md` — a skill that tells agents how to keep the list
  with `bb cyberpunk-terminal`. BB imports it into agent threads automatically.
...
`bb cyberpunk-terminal add "Ship it"` in a terminal. The page updates at once.
...
```
bb plugin reload cyberpunk-terminal
```
...
```
bb plugin config cyberpunk-terminal
bb plugin config cyberpunk-terminal set showDone false
bb plugin reload cyberpunk-terminal
```
