...
- An **Example todos** page in the left sidebar that adds, completes, and
  removes todos.
- A `bb cyberpunk-terminal` command that does the same from a terminal.
...
The bundled skill tells an agent to read the list with `bb cyberpunk-terminal list`, add
one todo at a time with `bb cyberpunk-terminal add`, and close finished work with
`bb cyberpunk-terminal done`.
