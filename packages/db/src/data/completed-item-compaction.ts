...
const candidateFields = sql`id, sequence, type, item_id AS itemId, item_kind AS itemKind,
  ...
  environment_id AS environmentId, octet_length(data) AS dataBytes`;
...
export function advanceCompletedItemCompaction(
  ...
  const pageSize = Math.min(32, Math.max(1, Math.floor(args.limit / 4)));
  ...
  for (const candidate of candidates) {
    if (args.limit - scanned < 8 || processedBytes >= INPUT_BYTE_BUDGET) break;
    ...
    if (
      db
        .select({ id: completedItemHistories.completionId })
        .from(completedItemHistories)
        .where(eq(completedItemHistories.completionId, candidate.id))
        .get() !== undefined
    ) {
      scanned++;
      continue;
    ...
    for (const type of [
      ...
      const rows = db.all<Candidate>(sql`SELECT ${candidateFields}
        ...
        WHERE thread_id = ${args.threadId} AND turn_id = ${candidate.turnId}
          ...
          ${type === "item/started" || type === "item/completed" ? sql`AND item_kind = ${kind}` : sql``}
    ...
    const start = records.find((row) => row.type === "item/started");
    const deltaRecords = records.filter((row) => row.type !== "item/started");
    const [delta, secondDelta] = deltaRecords;
    const data = encodeHistory(records);
    db.insert(completedItemHistories)
      .values({
        completionId: candidate.id,
        threadId: args.threadId,
        startId: start?.id ?? null,
        startSequence: start?.sequence ?? null,
        deltaId: delta?.id ?? null,
        deltaSequence: delta?.sequence ?? null,
        deltaType: delta?.type ?? null,
        secondDeltaId: secondDelta?.id ?? null,
        secondDeltaSequence: secondDelta?.sequence ?? null,
        secondDeltaType: secondDelta?.type ?? null,
        data,
      })
      .run();
    const result = db
