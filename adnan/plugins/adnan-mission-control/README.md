# bb-plugin-adnan-mission-control

A BB plugin that turns BB into a personal engineering cockpit (Adnan Mission
Control). Through Phase 5 it ships:

- `server.ts` — the backend: a mission-state store in `bb.storage.kv` with
  per-field provenance (agent-reported / user-confirmed), an RPC data plane
  for the panel, thread enrichment from BB's public SDK (model, reasoning,
  token usage, context pressure, pending interactions, last action), a
  per-provider capability probe, lifecycle-event subscriptions republished
  as realtime signals, the `report_mission_state` agent tool, and the
  `bb mission-control` CLI.
- `app.tsx` — the frontend: a **Mission Control** sidebar panel
  (`app.slots.navPanel`) with Overview and Agents views reached through
  sub-path routing.
  `components/quick-actions.tsx` adds a per-thread action menu (open, steer,
  ask why, request review/verification, stop, delegate) — the only place
  the plugin mutates a thread, and only on a direct click.
- `skills/mission-control/SKILL.md` — tells agents when and how to report
  workflow state. BB imports it into agent threads automatically.
- `PLUGIN_OVERVIEW.md` — the store listing text.

Try it: install the plugin, open **Mission Control** in the sidebar, then
run `bb mission-control set intent=FIX risk=high` in a terminal and watch
the Overview update.
...
```
bb plugin reload adnan-mission-control
```
...
```
bb plugin config adnan-mission-control
bb plugin config adnan-mission-control set showDone false
bb plugin reload adnan-mission-control
```
